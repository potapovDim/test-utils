  ...
  if(global.allure) {
    global.allure.step(
      `METHOD: ${reqMethod} | URL: ${reqUrl}`, () => {
        global.allure.attachment('Request Headers', JSON.stringify(reqHeaders, null, '\t'), 'application/json')
        if(!(reqBody instanceof FormData)) {
          global.allure.attachment('Request Payloads', itemToString(reqBody), 'application/json')
        }
        global.allure.attachment('Response Status', `${status}`, 'application/json')
        global.allure.attachment('Response Headers', `${JSON.stringify(headers, null, '\t')}`, 'application/json')
        global.allure.attachment('Response Body', itemToString(body), 'application/json')
      }
    )
  }
}
